...
| `CloudLibraryScanIntegrationTests` (gated) | Read-only walk of the REAL iCloud library on this machine/device: every sidecar and audio header goes through the gate, the pass must finish in under 5 s (the watchdog budget is 10 s) and must not pull a single audio file down. On the iPhone: 49 recordings, 0.13 s, no audio materialized |
