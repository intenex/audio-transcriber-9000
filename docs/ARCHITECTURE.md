...
- **Trailing-silence trim** (`Services/Shared/TrailingSilenceTrimmer.swift` + `RecordingStore.trimTrailingSilence`): drops the stretch at the end of a finished recording where nothing was captured. The cut point comes from the SAME `SilenceDetector` policy as the recorder's auto-stop (one definition of silence in the app) plus 15 s of padding after the last sound; tails under 60 s aren't worth a rewrite and return no plan. The cut itself is an `AVAssetExportSession` **passthrough** export (no re-encode, no quality loss), falling back to an `AVAudioFile` rewrite for containers passthrough won't take. The store writes the replacement into the spool, verifies its duration (±2 %), then swaps it in **at the same URL** — the stem is unchanged, so every sidecar keeps matching and transcript timings (all of which precede the cut) stay valid. Refused while the recording is `.processing` or is the active capture. Available from the recording's context menu on both platforms; UserDefaults `autoTrimTrailingSilence` (default **false**) runs it automatically on new recordings before transcription is enqueued (`AppBootstrap.onRecordingAdded`).
- **Unfinalized leftovers are quarantined, never adopted**: an interrupted stop/merge leaves a container with audio data but no index. `sweepSpool` checks `audioDuration(for:) > 0` before salvaging into the library and otherwise moves the file to `Application Support/AudioTranscriber/Unfinished/` (kept, never deleted). Without this a 619 MB unplayable 0-second "recording" appears in the library — it did, once.
...
- `CloudSyncManager` (@Observable @MainActor): resolves the container ONCE off-main (`bootstrap()`) and caches the path in UserDefaults `iCloudContainerPath` so `RecordingStore.resolveStorageDirectory` can branch synchronously; external change batches → debounced `reloadFromStorageDirectory()` + speaker-library reload + conflict sweep; per-item `state(for:)` + `stateVersion` for row badges. Batches consisting only of this app's own recent writes (`AtomicFile.isRecentSelfWrite`, 20 s window keyed by file name) are dropped without a reload — the echo is indistinguishable from a foreign edit otherwise, and reloading on it rebuilt the library from disk mid-operation. A reload that does happen preserves live state: `RecordingStore.inFlightTranscriptionIDs` (claimed by `TranscriptionService` from enqueue until the job ends) keeps running jobs `.processing` instead of letting the repair pass demote them.
